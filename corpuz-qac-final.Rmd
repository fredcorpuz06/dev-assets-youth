...
sum_plot <- ggplot(all_trt_effects) +
  geom_point(aes(x = trt, y = trt_diff, color = sig), size = 7) +
  geom_point(aes(x = trt, y = trt_low, color = sig), size = 4, shape = 18) +
  geom_point(aes(x = trt, y = trt_high, color = sig), size = 4, shape = 18) +
  geom_hline(yintercept = 0, size = 2, linetype = 'dashed', color = 'gray') +
  labs(title = 'Treatment Effects on No. of Thriving Indicators ',
       x = 'Treatment Type (Actionable External Asset)',
       y = 'Difference between groups caused by Treatment',
       color = 'p-value') +
